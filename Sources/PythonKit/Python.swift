...
private func isType(_ object: PythonObject,
  ...
  return UnsafeRawPointer(pyObject) == UnsafeRawPointer(&_Py_TrueStruct)
}

extension Bool : PythonConvertible, ConvertibleFromPython {
  public init?(_ pythonObject: PythonObject) {
    let _PyBool_Type = UnsafeMutableRawPointer(&PyBool_Type).assumingMemoryBound(to: PyObject.self)
    guard isType(pythonObject, type: _PyBool_Type) else { return nil }

    let pyObject = pythonObject.ownedPyObject
    defer { Py_DecRef(pyObject) }

    self = pyObject == UnsafeMutableRawPointer(&_Py_TrueStruct).assumingMemoryBound(to: PyObject.self)
  }
